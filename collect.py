...
# 237 конфигов замерились за 27 секунд, так что это не узкое место — берём с большим запасом.
CANDIDATE_LIMIT = 400
PER_COUNTRY_CANDIDATES = 40

# Скорость используется только для порядка, но НЕ для отбраковки.
#
# Был порог 2 Мбит/с, и он оказался вреден. Замер идёт из дата-центра в США, а пользователь
# сидит в России: у этих двух точек разные маршруты, разная фильтрация и разная загрузка узлов.
# Живой пример — Shadowsocks 140.174.184.5:8388: из раннера он не добрал до порога и был
# выброшен, а с телефона выдаёт огромную скорость. Отбрасывать по чужому замеру значит терять
# именно то, что работает у пользователя.
#
# Поэтому: наверх — то, что оказалось быстрым при замере, вниз — незамеренное, но в списке
# остаётся всё, что вообще откликнулось. Кто из них быстр на самом деле, решит проверка
# с телефона и балансировщик.
MIN_MBPS = 0.0
...
def cmd_rank() -> None:
    ...
    # Порядок один для обоих списков: сначала то, что замер показал быстрым, затем незамеренное
    # по отклику. Отбраковки по скорости больше нет — см. пояснение у MIN_MBPS.
    def order(config: Config):
        return (-config.mbps, config.latency_ms)

    if speeds:
        fast = sum(1 for c in configs if c.mbps > 0)
        log(f"Замер дал скорость у {fast} из {len(configs)}; остальные ниже, но в списке остаются")
    else:
        log("Замера нет — порядок только по отклику")

    publish(
        select(regular, PER_COUNTRY_LIMIT, TOTAL_LIMIT, key=order),
        OUTPUT_FILE, OUTPUT_META, "Обычные", required=True,
    )
    # Белые списки не обязательны: если источников для них в sources.txt нет, файл просто не
    # трогаем, а прогон считается успешным.
    publish(
        select(whitelisted, PER_COUNTRY_LIMIT, TOTAL_LIMIT, key=order),
